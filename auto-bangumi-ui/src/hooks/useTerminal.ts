...
import type { InstanceDetail } from '@/api/types/mcs/index.ts';
import { GLOBAL_INSTANCE_NAME, INSTANCE_STATUS_CODE } from '@/api/types/mcs/index.ts';
import { setUpTerminalStreamChannel } from '@/api/modules/mcs/protected/instance';
...
export function useTerminal() {
  ...
  const execute = async () => {
    ...
    socket.on('instance/stdout', (packet: any) => events.emit('stdout', packet?.data));
    socket.on('stream/detail', (packet: any) => {
  ...
  let statusQueryTask: ReturnType<typeof setInterval>;
  onMounted(() => {
